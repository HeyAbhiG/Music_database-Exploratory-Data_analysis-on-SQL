# Music_database-Exploratory-Data_analysis-on-SQL
## Project Overview

## Business Problem 
### The company seeks to leverage its sales transaction data, customer profiles, and music_database to address the following key business problems:
-  **Album Performance Variability**: Identifying which albums are performing well in terms of sales and which are not.
                                        This insight is crucial for music's album database management and marketing focus.
-  **Customer Segmentation:**         The company lacks a clear understanding of its Customer Segment. customer base segmentation.
                                      Effective segmentation is essential for targeted marketing and enhancing customer satisfaction.
- **Customer Behaviour Analysis:**    Understanding patterns in customer behavior, including repeat purchases and loyalty indicators,
                                      is critical for tailoring customer engagement strategies and improving retention rates.

 ## Objective 
- To utilize SQL queries for exploratory data analysis to ensure data quality and gain initial insights.
- To identify high and low sales products to optimize inventory and tailor marketing efforts.
- To segment customers based on their purchasing behavior for targeted marketing campaigns. Create Customer segments as per client requirements.
- To analyze customer behaviour for insights on repeat purchases and loyalty, informing customer retention strategies.

## Steps:
#### 1. Data Exploration and Cleaning:
- **Record Count** : Determine the total_number of records in all the data_set.
- **Customer Count**: Find out how many unique customers in the data_set.
- **Music_Category Count**: Identifies the different categories and their contribution on  Total_Sales.
- **Album_Price discrepancies**: Identify Price_discrepancies  of products on different table and Correct it 
- **Null Count**: Identify the Null_values in different data_sets and handle it properly .

### 2.Data Analysis 
- We have solved a good number of reasearch questions as per our stake-holders/client discussion

### 3. Findings
- **Customer Insights :**	Find out customer who repeating purchase and spending most in our site/stores so we can provide
                          them discount coupon and better service as their our most loyal customers. 
- **High-Value Transactions:** Several transactions had a total sale amount greater than 1000, indicating premium purchases.
- **Product Performance :** Analyze the Product performance based on the total_sales which help us identify product_catefory which needed to be promote
- **Inventory Management:** After analysing data set, we get to know the products which is giving us highest revenue and most in demand for better inventory management.
- 	Home and Kitchen category contribute highest number both in terms of  total_sales and unit_sold.
    
### Dataset Used
<a href="https://github.com/HeyAbhiG/SQL-Project-on-Retail-Analytics/blob/main/customer_profiles.csv">Customer_Profiles<a/>
<a href="https://github.com/HeyAbhiG/SQL-Project-on-Retail-Analytics/blob/main/product_inventory.csv">Product_inventory<a/>
<a href="https://github.com/HeyAbhiG/SQL-Project-on-Retail-Analytics/blob/main/sales_transaction.csv">sales_transaction<a/>
